...
export function groupSaleHistory(sales: Sale[], search = '', from = '', to = '') {
  ...
  for (const sale of sales) {
    if (sale.deleted || sale.lenderAction || sale.directTrade?.status === 'cancelled' || sale.date < lower || sale.date >= upper) continue
    const hay = toLatinDigits(`${sale.customerName || 'مشتری نقدی'} ${commercialSaleLines(sale).map(line => `${line.productName} ${line.size} ${line.color}`).join(' ')}`).toLowerCase()
