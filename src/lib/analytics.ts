...
const lineProfit = (l: CommercialSaleLine) => (l.unitPrice - (l.unitCost ?? 0)) * l.qty

/** مفاد یک فاکتور: مجموع خطوط منهای تخفیف */
export function saleProfit(s: Sale): number {
  if (s.directTrade?.status === 'cancelled') return 0
...
export function retailVsWholesale(sales: Sale[], returns: ReturnDoc[] = []): { retail: Totals; wholesale: Totals } {
  ...
  for (const s of sales) {
    ...
    t.pairs += commercialSaleLines(s).reduce((a, l) => a + l.qty, 0)
    t.count += 1
  }
...
export function byModel(sales: Sale[]): ModelRow[] {
  ...
  for (const s of sales) {
    ...
    const sub = lines.reduce((a, l) => a + l.qty * l.unitPrice, 0)
