...
try {
  ...
  for (const [type, cashPaid, creditAmount] of [['home', 100, 0], ['personal', 0, 100]]) {
    ...
    assert.equal(await dialog.getByRole('button', { name: 'اصلاح سند', exact: true }).count(), 0, 'does not expose unsupported correction')
    await dialog.getByRole('button', { name: 'حذف سند اشتباهی', exact: true }).click()
    await dialog.getByRole('button', { name: 'تأیید حذف همین سند', exact: true }).click()
    await dialog.waitFor({ state: 'hidden' })
  ...
  console.log('PASS: list/calendar details, cancel, correction entry, rollback/retry, cash/credit/mixed deletion, audit retention, read-only, responsive layout')
} finally {
  await browser?.close()
  server.kill()
}
